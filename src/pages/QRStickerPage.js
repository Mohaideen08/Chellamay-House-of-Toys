              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: 7, fontWeight: 900, color: "#000" }}
                    >
                    ...
                    <div
                      style={{ fontSize: 5.5, color: "#333", fontWeight: 700 }}
                    >
                      {now}
                    </div>
                  </div>
...
const QRStickerPage = () => {
  ...
  const handlePrint = useCallback(() => {
    ...
    if (!win) {
      showSnackbar("Pop-up blocked – allow pop-ups and try again", "warning");
      return;
    }
    ...
    const stickers = itemsSnap.flatMap((item) =>
      Array.from({ length: item.qty }, (_, i) => {
        ...
        const qrEl = document.querySelector(
          `[data-qr-id="${item.product.id}-${i}"] svg`,
        );
    ...
    // 5-inch roll: 135mm wide, sticker 42.3mm × 25mm (3 per row, 3mm gap)
    const rowCount = Math.ceil(stickers.length / 3);
    const pageHeightMm = rowCount * 25; // each row = 25mm, no row gap
    const styles = `
      @page { size: 135mm ${pageHeightMm}mm; margin: 0; }
      * { box-sizing: border-box; margin: 0; padding: 0; }
      html, body { font-family: Arial, sans-serif; background: #fff; width: 135mm; overflow: visible; }
      .grid { display: grid; grid-template-columns: repeat(3, 42.3mm); gap: 3mm; width: 130.9mm; }
      .sticker { width: 42.3mm; height: 25mm; overflow: hidden; display: flex; flex-direction: column; padding: 0.8mm 1.2mm; background: #fff; page-break-inside: avoid; break-inside: avoid; }
      ...
      .shop { font-size: 6pt; font-weight: 700; color: #111; letter-spacing: 0.2px; margin-bottom: 0.8pt; text-align: center; }
      ...
      .pname { font-size: 6.5pt; font-weight: 900; color: #111; line-height: 1.2; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
      .price { font-size: 8pt; font-weight: 900; color: #000; white-space: nowrap; }
      .code { font-size: 7pt; font-weight: 900; color: #333; font-family: monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .dt { font-size: 5pt; color: #111; font-weight: 900; }
  ...
  const toggleAll = () => {
    if (selected.length === filtered.length) {
      ...
      ids.forEach((id) => {
        if (!quantities[id]) {
          const stock =
            products.find((p) => p.id === id)?.current_quantity ?? 1;
  ...
  const openQtyDialog = () => {
    if (selectedItems.length === 0) {
      showSnackbar("Select at least one product", "warning");
      return;
    }
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography
              sx={{ color: "rgba(255,255,255,0.72)", fontSize: "0.72rem" }}
            >
      ...
      >
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1.2}>
            <Typography
              sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#1A1A2E" }}
            >
              Select Products
            </Typography>
            {!loading && (
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                <TableCell
                  ...
                  sx={{
                    background:
                      "linear-gradient(135deg, rgba(var(--color-primary-rgb),0.09) 0%, rgba(var(--color-secondary-rgb),0.09) 100%)",
                  }}
                >
                  <Checkbox
                    indeterminate={
            ...
            <TableBody>
              ...
              {filtered.map((product, index) => {
                ...
                const barcodeCode =
                  product.barcode || product.item_code || String(product.id);
                return (
                  <TableRow
                    key={product.id}
                    ...
                    <TableCell sx={{ maxWidth: 200 }}>
                      <Typography
                        sx={{
                          ...
                          color: isSelected ? "#AD1457" : "text.primary",
                        }}
                        noWrap
                      >
                        {product.name}
        ...
        <DialogTitle sx={{ p: 0 }}>
          ...
          >
            ...
            >
              ...
              <Box>
                <Typography
                  sx={{ fontWeight: 800, fontSize: "1.05rem", lineHeight: 1.2 }}
                  component="div"
                >
                  Set Sticker Quantities
                </Typography>
                <Typography sx={{ fontSize: "0.74rem", opacity: 0.78 }}>
        ...
        <DialogContent sx={{ p: 0 }}>
          ...
          <Box
            sx={{
              ...
              py: 2,
              background:
                "linear-gradient(135deg, rgba(var(--color-primary-rgb),0.06) 0%, rgba(var(--color-secondary-rgb),0.06) 100%)",
        ...
        >
          ...
          <Button
            ...
            sx={{
              ...
              borderRadius: "10px",
              fontWeight: 600,
              borderColor: "rgba(var(--color-secondary-rgb),0.4)",
              color: "secondary.main",
              "&:hover": {
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            borderRadius: "10px",
            overflow: "hidden",
            boxShadow: "0 24px 64px rgba(var(--color-primary-rgb),0.25)",
          },
        }}
      >
        <Box
