...
const BillingPage = () => {
  ...
  const handlePrintRawBT = useCallback(() => {
    ...
    dash();
    txt(col('Items:' + totalItemsCount + ' Qty:' + totalQty, 28) + col(sub.toFixed(2), 20, true)); nl();
    dash();
    txt(col('TaxableAmt', 16) + col('CGST', 16, true) + col('SGST', 16, true)); nl();
    txt(col(sub.toFixed(2), 16) + col(cgstAmt.toFixed(2), 16, true) + col(sgstAmt.toFixed(2), 16, true)); nl();
    dash();
